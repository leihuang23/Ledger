...
- Honest positioning: Ledger is a production-shaped system that demonstrates investigation, cited evidence, auditable run traces, and approval-gated risky actions. Disclose synthetic data, optional Stripe test-mode sandbox evidence, read-only public demo, the deterministic classifier as the validation gate for any LLM diagnosis, and mock external actions behind approval. Do not imply merchant adoption or live commerce.
...
- At least 4 of 5 eval scenarios correctly identify the intended root cause (the seed includes 6 eval scenarios; the baseline clears all six).
...
- The current backend is organized by domain under `apps/api/app/`: `accounts`, `agent`, `agents`, `approvals`, `core`, `dashboard`, `db`, `evals`, `health`, `incidents`, `knowledge`, `llm`, `mcp`, `metrics`, `runs`, `stripe_adapter`, `support`, and `tools`. `mcp` is a thin FastMCP adapter over the shared tool registry (no second tool system): `read_data` tools are open, governed tools reuse the `DEMO_OPERATOR_TOKEN` gate with demo fail-closed semantics, and high-risk actions route into the existing approval service. The Next.js frontend under `apps/web/app/` mirrors the operational surfaces: dashboard, incidents, agent runs, approvals, accounts, support tickets, knowledge search, evals, and the control-plane pages (agents, tools, runs, dashboard).
- LangGraph is the recommended orchestrator. The investigation workflow in `apps/api/app/agent/workflow.py` is dual-mode, selected by LLM capability and the `AGENT_LOOP_ENABLED` gate at run time: with `LLM_PROVIDER=none` it stays a fixed linear DAG (reproducible evals); with a real provider it delegates evidence gathering to the bounded ReAct loop in `apps/api/app/agent/loop.py` (LLM picks tools and order, capped by `AGENT_MAX_ITERATIONS`; set `AGENT_LOOP_ENABLED=false` to pin a provider back to single-shot diagnosis (a deterministic evidence sweep plus one gated diagnosis call)). Preserve the loop's safety invariants: tool policy checked before dispatch, LLM arguments sanitized to known ids (account ids, and only genuinely failed invoice ids), duplicate calls short-circuited, honest degradation to the deterministic sweep, policy-blocked tools receive the same `tool_disabled` fallback payloads as the deterministic pipeline (keeping the classifier's uncertainty guard), and the deterministic classifier remains the adoption gate for any LLM diagnosis. Do not add dynamic graph branching or self-modifying tool registries beyond this design.
