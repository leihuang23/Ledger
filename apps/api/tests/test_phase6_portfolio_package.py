...
def test_public_demo_launch_artifacts_and_checklist() -> None:
    """Anonymous public portfolio path is documented and scriptable without secrets."""
    ...
    assert "DEMO_OPERATOR_TOKEN" not in script or "never prints token" in script.lower() or "Never prints token" in script

    e2e_workflow = ROOT.joinpath(".github/workflows/e2e.yml").read_text(encoding="utf-8")
    ...
    assert ROOT.joinpath("apps/web/e2e/public-portfolio-browse.spec.ts").is_file()
