...
| S-5 — Dashboard shows status, trace, latency, and estimated cost | dashboard API tests, `/dashboard`, and demo screenshot |
...
- Deterministic eval CLI: Project 1 compatibility remains pinned to immutable v1 and passes at least `4/5`; the explicit Phase 6 comparison remains the release baseline at `6/6` versus `5/6` for the intentionally document-search-disabled candidate.
...
- Demo assets: both PNGs were inspected at `1440x900`; the WebM duration is `195.8` seconds.
